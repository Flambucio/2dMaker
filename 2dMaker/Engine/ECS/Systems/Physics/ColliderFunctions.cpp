...
bool ColliderFunctions::MousePointerCollisionWithRec(const Rectangle& rec)
{
	double mouseX = 0;
	double mouseY = 0;
	glfwGetCursorPos(Window::GetMainWindow(), &mouseX, &mouseY);

	if (rec.x <= mouseX && mouseX <= rec.x + rec.width &&
		rec.y <= mouseY && mouseY <= rec.y + rec.height)
	{
		return true;
	}
	return false;
...
bool ColliderFunctions::CheckCollision(const Rectangle& rec1, const Rectangle& rec2)
{
...
bool ColliderFunctions::InViewport(Transform* transform)
{
...
bool ColliderFunctions::CheckCollisionWithName(Entity entityIn, EntityManager& em, std::string nameIn)
{
...
void ColliderFunctions::AdjustXStatic(Transform* transform, const Rectangle& rec, float dx)
{
	if (dx > 0)
	{
...
void ColliderFunctions::AdjustXDynamic(Transform* transform1, Transform* transform2, float dx1, float dx2)
{
...
void ColliderFunctions::AdjustYStatic(Transform* transform, const Rectangle& rec, float dy)
{
	if (dy > 0)
	{
...
void ColliderFunctions::AdjustYDynamic(Transform* transform1, Transform* transform2, float dy1, float dy2)
{
